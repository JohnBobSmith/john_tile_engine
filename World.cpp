...
//Load our detail map.
bool World::loadDetailMap(const std::string& tileset, sf::Vector2u tileSize,
                                    unsigned int width, unsigned int height)
{
    //Do we have our m_detailtex texture?
    if (!m_detailtex.loadFromFile(tileset)) {
        return false;
    }

    //Fit the vertex array to the texture
    m_detailverts.setPrimitiveType(sf::Quads);
    m_detailverts.resize(width * height * 4);
    ...
    for (unsigned int i = 0; i < width; ++i) {
        for (unsigned int j = 0; j < height; ++j) {
            ...
            //Find its position on the texture
            int tu = tileNumber % (m_detailtex.getSize().x / tileSize.x);
            int tv = tileNumber / (m_detailtex.getSize().x / tileSize.x);

            //Get a pointer to the current tile's quad
            sf::Vertex* quad = &m_detailverts[(i + j * width) * 4];
